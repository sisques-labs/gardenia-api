...
- **THEN** the `TRUNCATE_TABLES` constant in `test/helpers/db-reset.ts` MUST be updated to include the new table
...
ESLint MUST enforce `no-restricted-imports` for `@nestjs/testing` in `src/**/*.spec.ts`.
...
- **THEN** ESLint MUST fail with a clear message
- **AND** the change MUST be rejected by architecture review
...
At least **two** integration specs MUST validate tenant/space isolation at the repository persistence boundary.
...
### Requirement: Schema parity via migrations

All database-backed test suites (integration and E2E) MUST apply schema via TypeORM migrations (`migration:run`) instead of `synchronize: true`.

#### Scenario: Integration suite uses migrations

- **WHEN** the integration test harness initializes the database
...
### Requirement: Optional Testcontainers for local development
...
The project MUST update `openspec/config.yaml` to reflect the three test layers and require applicable test coverage when applying changes.
...
- **WHEN** the testing infrastructure change is complete
