...
- **Location `src/core/metrics/`**: metrics are cross-cutting operational infra with no domain model, sitting beside the existing `ObservabilityModule` (Sentry). Not a bounded context → not under `src/contexts/`.
