...
The API has NO runtime metrics. We have a liveness probe (`GET /api/health`), but there is no way to answer "how many requests, how slow, how many commands/queries are failing, is the event loop saturated?". Operators are blind to throughput, latency, and saturation between deploys. We need a Prometheus-scrapeable `GET /api/metrics` endpoint exposing process, HTTP, and CQRS metrics so dashboards and alerts can be built. Needed now to make the service observable in production.
...
- **CQRS metrics** via a startup-time wrap of the shared `CommandBus` / `QueryBus` `execute` plus an `EventBus` stream subscription: per-command/query duration histogram + counter labelled `type`, `kind` and `status` (`success` | `error`), and an event-published counter labelled `event`.
- New `MetricsModule` under `src/core/metrics/`, registered in `AppModule`.
- Unit specs for the interceptor + CQRS metrics service + controller, and an E2E spec asserting the endpoint is public and emits the expected metric families.
...
- **Location `src/core/metrics/`**: metrics are cross-cutting operational infra with no domain model, sitting beside the existing `ObservabilityModule`. Not a bounded context → not under `src/contexts/`.
...
- **CQRS via wrapping the shared bus singletons (not per-handler edits, not DI subclasses)**: `@nestjs/cqrs@10` has no native command middleware AND registers handlers onto the global, exported `CommandBus`/`QueryBus` singletons. A `{ provide: CommandBus, useClass: ... }` override would create a *second* instance with no handlers registered, so resolvers would keep using the original. Instead a `CqrsMetricsService` wraps the `execute` method of the shared instances at `onModuleInit`, and subscribes to the `EventBus` stream for publish counts. Handlers keep injecting `CommandBus`/`QueryBus` unchanged — zero edits across the contexts.
...
| Wrapping `execute` diverges from `@nestjs/cqrs` internals on upgrade | Low | Wrap only the public `execute` surface + `EventBus` stream; unit specs lock behaviour |
...
Remove `MetricsModule` from `src/app.module.ts`, delete `src/core/metrics/`, and drop the two dependencies from `package.json`. Self-contained — no migrations, no schema or shared-state changes. Removing the module unhooks the `execute` wrapping, restoring the stock `@nestjs/cqrs` buses transparently.
...
- [ ] Handlers across contexts remain unedited (shared buses wrapped at startup).
