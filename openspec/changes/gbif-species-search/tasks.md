...
- [x] 1.1 Confirm the live name of the unique constraint on
      `plant_species.scientific_name` (established by earlier migrations)
      before writing `DROP CONSTRAINT`.
- [x] 1.2 Create `src/database/migrations/{next-timestamp}-TrimPlantSpeciesToGbif.ts`:
...
- [x] 2.1 Delete `domain/value-objects/plant-species-description/` (+ spec),
...
- [x] 2.2 Create `domain/value-objects/plant-species-gbif-key/plant-species-gbif-key.value-object.ts`
      (`PlantSpeciesGbifKeyValueObject` extends `NumberValueObject`, positive
      integer) + spec.
- [x] 2.3 Create `domain/events/field-changed/plant-species-gbif-key-changed/plant-species-gbif-key-changed.event.ts`
      (`PlantSpeciesGbifKeyChangedEvent`).
- [x] 2.4 Update `domain/interfaces/plant-species.interface.ts`,
      `domain/primitives/plant-species.primitives.ts`: drop
      `description`/`imageUrl`; add `gbifKey: number | null`.
- [x] 2.5 Update `domain/aggregates/plant-species.aggregate.ts` (+ spec): drop
      `_description`/`_imageUrl` + their change methods; add `_gbifKey` +
      `changeGbifKey()`; update constructor/`update()`/`toPrimitives()`/getters.
- [x] 2.6 Update `domain/builders/plant-species.builder.ts` (+ spec):
      `withDescription`/`withImageUrl` → `withGbifKey`.
- [x] 2.7 Update `domain/view-models/plant-species.view-model.ts`: drop
      `description`/`imageUrl`, add `gbifKey`.
- [x] 2.8 Delete `domain/exceptions/plant-species-name-already-exists.exception.ts`;
...
- [x] 3.1 Delete `application/commands/enrich-plant-species/` (+ spec),
...
- [x] 3.2 Delete `application/services/write/assert-plant-species-name-available/`
...
- [x] 3.3 Update `application/commands/create-plant-species/` and
      `update-plant-species/` (+ specs): field swap
      (`description`/`imageUrl` → `gbifKey`), wire the new assert service.
- [x] 3.4 Create `application/commands/find-or-create-plant-species-by-gbif-key/`:
...
- [x] 3.5 Create `application/ports/gbif-species-search.port.ts`
      (`GBIF_SPECIES_SEARCH_PORT`, `IGbifSpeciesSearchPort.suggest(name,
      limit)`).
- [x] 3.6 Create `application/queries/gbif-species-search/gbif-species-search.query.ts`
      + `.handler.ts` + `.handler.spec.ts` (mock port: happy path, empty
      result, limit clamping).
- [x] 3.7 Update `domain/repositories/read/plant-species-read.repository.ts` +
      TypeORM implementation: add `findByGbifKey(gbifKey: number)`.
- [x] 3.8 Update `transport/graphql/enums/plant-species-queryable-field.enum.ts`
...
- [x] 4.1 Update `infrastructure/persistence/typeorm/entities/plant-species.entity.ts`:
...
- [x] 4.2 Update `infrastructure/persistence/typeorm/mappers/plant-species-typeorm.mapper.ts`
      (+ spec): field swap.
- [x] 4.3 Update `infrastructure/persistence/typeorm/repositories/plant-species-typeorm-read.repository.ts`
      (+ write repo if needed): implement `findByGbifKey`.
- [x] 4.4 Delete `infrastructure/adapters/gbif-plant-species-import.adapter.ts`
...
- [x] 4.5 Create `infrastructure/adapters/gbif-species-suggest.adapter.ts`
...
- [x] 5.1 Trim GraphQL create/update DTOs + response DTO (drop
      ...
      (+ spec).
- [x] 5.2 Create `transport/graphql/dtos/requests/gbif-species-search.request.dto.ts`
      ...
      (+ spec).
- [x] 5.3 Trim REST create/update/response DTOs; add `GET
      /plant-species/search` to `plant-species.controller.ts` (+ spec).
- [x] 5.4 Trim MCP `plant-species-create.schema.ts`/`-update.schema.ts`;
...
- [x] 5.5 Update `plant-species.module.ts`: remove enrich/import handler +
...
- [x] 6.1 Update `application/commands/create-plant/create-plant.command.ts`
      and `update-plant/update-plant.command.ts`: `plantSpeciesId?` →
      `gbifSpeciesKey?: number`, `speciesScientificName?: string` (paired).
- [x] 6.2 Update `application/ports/plant-species.port.ts`: add
      `findOrCreateByGbifKey`.
- [x] 6.3 Update `infrastructure/adapters/plant-species.adapter.ts` (+ spec):
      implement `findOrCreateByGbifKey` (dispatch
      `FindOrCreatePlantSpeciesByGbifKeyCommand` via `CommandBus`).
- [x] 6.4 Delete `application/services/write/assert-plant-linked-species-exists/`
      (+ spec) and `domain/exceptions/plant-linked-species-not-found.exception.ts`.
- [x] 6.5 Update `application/commands/create-plant/create-plant.handler.ts`
...
- [x] 6.6 Update `domain/builders/plant-species.builder.ts` (plants-local
      mirror), `domain/view-models/plant-species.view-model.ts`,
      `domain/primitives/plant-species-view-model.primitives.ts`: drop
      `description`/`imageUrl`, add `gbifKey`.
- [x] 6.7 Update GraphQL: `plant-create.request.dto.ts` /
...
- [x] 6.8 Update REST/MCP equivalents (`create-plant.dto.ts`,
      `update-plant.dto.ts`, `plant-rest-response.dto.ts`,
      `plant-create.schema.ts`, `plant-update.schema.ts`).
- [x] 6.9 Confirm no change needed to `PlantAggregate`, `plant.entity.ts`,
...
- [x] 7.1 Update `test/integration/plant-species/*.integration-spec.ts` for
      the trimmed schema + `findByGbifKey`.
- [x] 7.2 Update `test/e2e/plant-species/*.e2e-spec.ts` (REST + GraphQL) for
      trimmed fields, removed enrich/import endpoints, new search endpoint.
- [x] 7.3 Update `test/integration/plants/*.integration-spec.ts` and
      `test/e2e/plants/plants-rest.e2e-spec.ts` for the command input field
      swap and find-or-create behavior.
- [x] 7.4 Update `src/contexts/plant-species/README.md` and
...
- [x] 8.2 `pnpm tsc --noEmit` clean.
- [x] 8.3 `pnpm test` — full unit suite green.
- [ ] 8.4 `pnpm test:integration` — NOT RUN in this environment (no Postgres/Docker available); specs were updated for correctness but not executed against a live DB. Run before merging.
- [ ] 8.5 `pnpm test:e2e` — NOT RUN, same reason as 8.4. Run before merging.
- [x] 8.6 `pnpm test:cov` — full unit suite green (419 suites / 1731 tests).
- [x] 8.7 Grep the repo for `description`/`imageUrl` references still tied to
