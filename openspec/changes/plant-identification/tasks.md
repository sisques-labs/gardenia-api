...
- [x] 5.2 Integration: `test/integration/plant-identification/` — tenant isolation on read/write repos, cascade delete of photos/candidates, `findByCriteria` filter/sort coverage. Not runnable locally (no Postgres/Docker in this sandbox) but exercised via CI on the PR, which caught two real bugs since fixed: (1) `save()` wasn't stamping the active `SpaceContext` space onto the row (see Phase 3.9 CI-fix note), (2) the new tables were missing from `test/helpers/db-reset.ts`'s `TRUNCATE_TABLES`, causing cross-test data leakage per `openspec/specs/testing-infrastructure/spec.md`'s own rule. Both fixed; awaiting a green CI re-run to confirm.
...
## Phase 7: PR review follow-up (code-convention pass)

- [x] 7.1 Extracted `identify-plant.handler.ts`'s upload/identify/resolve blocks into three `IBaseService`-implementing application services (`UploadIdentificationPhotosService`, `IdentifyPlantPhotosService`, `ResolvePlantSpeciesMatchService`); the handler is now pure orchestration. The provider-naming log message ("Calling PlantNet identification...") moved into `IdentifyPlantPhotosService` and was reworded to not name the provider (application layer must stay provider-agnostic — same principle as the earlier GBIF-agnostic refactor).
- [x] 7.2 New dedicated value objects: `PlantIdentificationProjectValueObject`, `PlantIdentificationScientificNameValueObject` (candidate), `PlantIdentificationResolvedScientificNameValueObject` (aggregate top-level), `PlantIdentificationRankValueObject`, `PlantIdentificationCommonNameValueObject`, `PlantIdentificationPhotoUrlValueObject`, `PlantIdentificationPhotoPositionValueObject` — replacing bare `StringValueObject`/`NumberValueObject` usage on aggregate fields.
- [x] 7.3 `PlantIdentificationStatusEnum` values uppercased (`RESOLVED`/`NO_MATCH`) for consistency with `care-log`/`inventory`'s enum convention. `PlantIdentificationOrganEnum` deliberately left lowercase — its raw values are sent verbatim to PlantNet's real API (`organs` form field), which is documented to expect lowercase; uppercasing would require a translation layer in the adapter for no benefit, so it was scoped out (see PR reply).
- [x] 7.4 `commonNames` normalized out of `plant_identification_candidates.common_names` (`text[]`) into a new child table `plant_identification_candidate_common_names` (own `id`, `candidate_id` FK `ON DELETE CASCADE`, `name`, `position`), mirroring the existing photos/candidates child-table pattern. Migration `1780000000025-CreatePlantIdentifications` edited in place (not yet deployed anywhere). Write repository inserts common-name rows after the candidate insert (needs the DB-generated `candidate_id`); read path loads and groups them alongside photos/candidates.
- [x] 7.5 Added `PlantNetApiKeyMissingException` (`infrastructure/exceptions/`) — `plantnet.config.ts`'s fail-fast check now throws a dedicated exception instead of a bare `Error`.
- [x] 7.6 Scoped out (left unchanged, replied on the PR thread instead): removing the ownership check on `CreatePlantFromIdentification` (kept — security-relevant, reviewer agreed to leave it), linking candidate `scientificName` to the `plant_species` catalog (conflicts with the established "don't persist unconfirmed candidates" ADR — flagged for reviewer), and `mimeType`/`filename` value objects "in nestjs-kit" (gardenia-api consumes `@sisques-labs/nestjs-kit` as a published package, not a workspace link — tracked as a separate PR in that repo, to be adopted here once released).
- [x] 7.7 Verified: `pnpm tsc --noEmit` clean, full `pnpm test` green (463 suites / 1877 tests, whole repo), `eslint --fix` on touched files.
