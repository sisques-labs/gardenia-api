...
- [ ] `detect.sh` derives `CURRENT` from git tags on all three branches.
- [ ] `docker-release.yml` creates a tag but does NOT commit version files on `develop`.
- [ ] `package.json` on develop/staging stays `0.0.0-dev` across release cycles.
- [ ] A `develop → staging` PR after a beta release has zero version-file conflicts.
