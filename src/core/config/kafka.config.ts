...
export interface IKafkaConfig {
  ...
  clientId: string;
  brokers: string[];
  /** Topic prefix — topics are `${topicPrefix}.${module}` (e.g. `gardenia-api.plants`). */
  topicPrefix: string;
  ssl: boolean;
...
export const kafkaConfig = registerAs('kafka', (): IKafkaConfig => {
  return {
    ...
    clientId: process.env.KAFKA_CLIENT_ID?.trim() || 'gardenia-api',
    brokers: parseBrokers(process.env.KAFKA_BROKERS),
    topicPrefix: process.env.KAFKA_TOPIC_PREFIX?.trim() || 'gardenia-api',
