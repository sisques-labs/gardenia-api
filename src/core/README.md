...
- **`ObservabilityModule`** — registers `CorrelationIdInterceptor` (assigns/propagates a per-request correlation id via `X-Request-Id`) and `HttpLoggingInterceptor` (logs method/path, status, and duration for every REST and GraphQL request) as global interceptors.
...
| **observability** | `observability/` | `ObservabilityModule` — request correlation id + HTTP access logging interceptors |
