...
describe('CqrsMetricsService', () => {
  ...
  describe('commands', () => {
    it('records a successful command with its class name and success status', async () => {
      const result = await commandBus.execute(new CreatePlantCommand());

      expect(result).toBe('command-result');
      const labels = {
        type: 'CreatePlantCommand',
        kind: 'command',
        status: 'success',
      };
    ...
    it('records an error status and rethrows when the handler fails', async () => {
      ...
      await expect(
        failingBus.execute(new CreatePlantCommand()),
  ...
  describe('queries', () => {
    it('records a successful query with kind=query', async () => {
      await queryBus.execute(new GetPlantQuery());
  ...
  describe('events', () => {
    it('counts a published event by its class name', () => {
      eventStream.next(new PlantCreatedEvent());

      expect(events.inc).toHaveBeenCalledWith({ event: 'PlantCreatedEvent' });
    });
