...
describe('MetricsController', () => {
  it('carries @SkipSpace() metadata on the index handler (public route)', () => {
    const skipSpace = Reflect.getMetadata(
      SKIP_SPACE_KEY,
      MetricsController.prototype.index,
    );

    expect(skipSpace).toBe(true);
  });
