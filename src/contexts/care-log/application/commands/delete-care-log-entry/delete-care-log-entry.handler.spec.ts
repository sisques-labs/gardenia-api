...
import { DeleteCareLogEntryCommandHandler } from './delete-care-log-entry.handler';
...
describe('DeleteCareLogEntryCommandHandler', () => {
  let handler: DeleteCareLogEntryCommandHandler;
  ...
  beforeEach(() => {
    ...
    handler = new DeleteCareLogEntryCommandHandler(
      writeRepository,
      assertExists,
      eventBus,
    );
  ...
  describe('happy path — owner deletes', () => {
    it('should call delete and publish events when owner deletes the entry', async () => {
      const aggregate = buildAggregate();
      assertExists.execute.mockResolvedValue(aggregate);

      const command = new DeleteCareLogEntryCommand({
        id: ENTRY_ID,
      ...
      expect(writeRepository.delete).toHaveBeenCalledTimes(1);
      expect(writeRepository.delete).toHaveBeenCalledWith(ENTRY_ID);
      expect(eventBus.publishAll).toHaveBeenCalledTimes(1);
    });
  });

  describe('space member deletes an entry they did not create', () => {
    it('should allow any requesting user to delete the entry', async () => {
      const aggregate = buildAggregate();
      assertExists.execute.mockResolvedValue(aggregate);
