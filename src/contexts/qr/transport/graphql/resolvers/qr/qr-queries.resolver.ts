...
export class QrQueriesResolver {
  ...
  @Query(() => QrResponseDto, { nullable: true })
  @UseGuards(JwtAuthGuard)
  async qrFindByPlantId(
    @Args('input') input: QrFindByPlantIdRequestDto,
  ): Promise<QrResponseDto | null> {
    this.logger.log(`Finding QR by plant id: ${input.plantId}`);

    const result = await this.queryBus.execute<
      QrFindByPlantIdQuery,
      QrViewModel | null
    >(new QrFindByPlantIdQuery({ plantId: input.plantId }));

    return result
      ? this.qrGraphQLMapper.toResponseDtoFromViewModel(result)
      : null;
  }
}
