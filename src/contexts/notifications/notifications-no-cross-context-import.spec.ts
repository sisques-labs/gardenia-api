...
describe('notifications bounded context — no cross-context imports', () => {
  it('has no import from another bounded context outside infrastructure/adapters', () => {
    ...
    const forbidden =
      /from\s+['"](@contexts\/care-schedule|@contexts\/inventory|@contexts\/users|@contexts\/spaces|@contexts\/plants|@contexts\/plant-species|@contexts\/care-log|@contexts\/harvests|@contexts\/planting-spots|@contexts\/weather|@contexts\/files)[/'"]/;
