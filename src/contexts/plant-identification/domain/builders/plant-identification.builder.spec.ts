...
const base = (): PlantIdentificationBuilder =>
  new PlantIdentificationBuilder()
    ...
    .withStatus(PlantIdentificationStatusEnum.RESOLVED)
    .withResolved({ gbifKey: 2882337, scientificName: 'Monstera deliciosa' })
    .withPhotos(PHOTOS)
...
describe('PlantIdentificationBuilder', () => {
  describe('build()', () => {
    ...
    it('leaves resolved fields null when withResolved(null)', () => {
  ...
  describe('validate()', () => {
    ...
    it('throws when status is missing', () => {
      expect(() =>
        base()
          .withStatus(undefined as unknown as PlantIdentificationStatusEnum)
          .build(),
      ).toThrow(FieldIsRequiredException);
    });
