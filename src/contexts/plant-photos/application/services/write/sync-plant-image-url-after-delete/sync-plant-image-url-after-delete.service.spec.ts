...
import { SyncPlantImageUrlAfterDeleteService } from './sync-plant-image-url-after-delete.service';
...
describe('SyncPlantImageUrlAfterDeleteService', () => {
  ...
  beforeEach(() => {
    ...
    service = new SyncPlantImageUrlAfterDeleteService(
      plantsPort,
      readRepository,
    );
  });
  ...
  it('resyncs to the next most recent photo when the deleted one was current', async () => {
    ...
    await service.execute(buildPhoto());
  ...
  it('does not throw when the resync fails', async () => {
    ...
    await expect(service.execute(buildPhoto())).resolves.toBeUndefined();
  });
});
