...
import { MarkPlantingSpotActiveCommand } from './mark-planting-spot-active.command';
import { MarkPlantingSpotActiveCommandHandler } from './mark-planting-spot-active.handler';
...
const buildAggregate = (): PlantingSpotAggregate =>
  new PlantingSpotAggregate({
    ...
    dimensions: null,
    soilType: null,
    status: new PlantingSpotStatusValueObject(PlantingSpotStatusEnum.FALLOW),
    fallowSince: new PlantingSpotFallowSinceValueObject(new Date('2025-06-01')),
...
describe('MarkPlantingSpotActiveCommandHandler', () => {
  let handler: MarkPlantingSpotActiveCommandHandler;
  ...
  beforeEach(() => {
    ...
    handler = new MarkPlantingSpotActiveCommandHandler(
      writeRepository,
      assertExistsService,
      eventBus,
    );
  ...
  describe('happy path — owner reactivates', () => {
    it('should mark the spot active, clear fallowSince, save and publish events', async () => {
      const aggregate = buildAggregate();
      assertExistsService.execute.mockResolvedValue(aggregate);

      const command = new MarkPlantingSpotActiveCommand({
        id: SPOT_ID,
      ...
      expect(aggregate.status.value).toBe(PlantingSpotStatusEnum.ACTIVE);
      expect(aggregate.fallowSince).toBeNull();
      expect(writeRepository.save).toHaveBeenCalledTimes(1);
      expect(eventBus.publishAll).toHaveBeenCalledTimes(1);
    });
  });

  describe('space member reactivates a spot they did not create', () => {
    it('should allow any requesting user to mark the spot active', async () => {
      const aggregate = buildAggregate();
      assertExistsService.execute.mockResolvedValue(aggregate);
