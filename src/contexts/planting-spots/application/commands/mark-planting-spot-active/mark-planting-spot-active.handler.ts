...
@CommandHandler(MarkPlantingSpotActiveCommand)
export class MarkPlantingSpotActiveCommandHandler
  extends BaseCommandHandler<
    MarkPlantingSpotActiveCommand,
    PlantingSpotAggregate
  >
  implements ICommandHandler<MarkPlantingSpotActiveCommand, void>
{
  private readonly logger = new Logger(
    MarkPlantingSpotActiveCommandHandler.name,
  );
  ...
  async execute(command: MarkPlantingSpotActiveCommand): Promise<void> {
    ...
    this.logger.log(
      `PlantingSpot marked active: ${command.id.value} by user: ${command.requestingUserId.value}`,
    );
  }
}
