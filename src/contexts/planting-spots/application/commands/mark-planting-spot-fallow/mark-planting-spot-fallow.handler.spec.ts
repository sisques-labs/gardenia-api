...
import { MarkPlantingSpotFallowCommandHandler } from './mark-planting-spot-fallow.handler';
...
describe('MarkPlantingSpotFallowCommandHandler', () => {
  let handler: MarkPlantingSpotFallowCommandHandler;
  ...
  beforeEach(() => {
    ...
    handler = new MarkPlantingSpotFallowCommandHandler(
      writeRepository,
      assertExistsService,
      eventBus,
    );
  ...
  describe('happy path — owner marks fallow', () => {
    it('should mark the spot fallow, save and publish events', async () => {
      const aggregate = buildAggregate();
      assertExistsService.execute.mockResolvedValue(aggregate);

      const command = new MarkPlantingSpotFallowCommand({
        id: SPOT_ID,
      ...
      expect(aggregate.status.value).toBe(PlantingSpotStatusEnum.FALLOW);
      expect(aggregate.fallowSince).not.toBeNull();
      expect(writeRepository.save).toHaveBeenCalledTimes(1);
      expect(eventBus.publishAll).toHaveBeenCalledTimes(1);
    });
  });

  describe('space member marks fallow a spot they did not create', () => {
    it('should allow any requesting user to mark the spot fallow', async () => {
      const aggregate = buildAggregate();
      assertExistsService.execute.mockResolvedValue(aggregate);
